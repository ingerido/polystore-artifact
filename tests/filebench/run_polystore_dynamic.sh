...
numactl --cpunodebind=0 env LD_PRELOAD=$POLYLIB_PATH/build/libpolystore_dynamic.so $FILEBENCH_PATH/filebench -f $FILEBENCH_PATH/workloads_polystore/fileserver_16.f
#numactl --cpunodebind=0 env LD_PRELOAD=$POLYLIB_PATH/build/libpolystore_dynamic.so $FILEBENCH_PATH/filebench -f $FILEBENCH_PATH/workloads_polystore/varmail_16.f
