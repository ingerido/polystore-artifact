...
// optimization, so skip the check of the max level.
  ...
  int64_t ParetoCdfInversion(double u, double theta_param, double k, double sigma) {
    double ret;
    if (k == 0.0) {
      ret = theta_param - sigma * std::log(u);
    } else {
      ret = theta_param + sigma * (std::pow(u, -1 * k) - 1) / k;
    }
