...
result_dir=$RESULTS_PATH/fig6_c_d/orthus/
...
ResetFiles() {
        for t in {0..31}
        do
                if [ ! -f $ORTHUS_DIR/thread_$t ]; then
                        touch $ORTHUS_DIR/thread_$t
                else
                        truncate -s 0 $ORTHUS_DIR/thread_$t
                fi
        done
}

# Generate Testing files in FAST and SLOW device
if [ ! -d $ORTHUS_DIR ]; then
        mkdir $ORTHUS_DIR
fi

FlushDisk

export HETERO_DIR=$ORTHUS_DIR
...
do
        ...
        do
                ...
                numactl --cpunodebind=0 $MICROBENCH_PATH/hetero_io_bench_transparent -r $thread -j $workloadid -s 4096 -t 1 -y 0 -x 0 -z 1g -f 1 -d 1 &> $output/result.txt

                echo "end $workload $cachesize"
                FlushDisk
                sleep 2
        done
