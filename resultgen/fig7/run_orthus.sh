...
result_dir=$RESULTS_PATH/fig7/orthus/
...
ResetFiles() {
        if [ -d $ORTHUS_DIR/bigfileset ]; then
                rm -rf $ORTHUS_DIR/bigfileset/*
        fi
}
...
do
        ...
        do
                ...
                $FILEBENCH_PATH/filebench -f $FILEBENCH_PATH/workloads_orthus/${workload}_${thread}.f &> $output/result.txt
