...
result_dir=$RESULTS_PATH/fig7/spfs/
...
ResetFiles() {
        if [ -d $SPFS_DIR/bigfileset ]; then
                rm -rf $SPFS_DIR/bigfileset/*
        fi
}
...
do
        ...
        do
                ...
                $FILEBENCH_PATH/filebench -f $FILEBENCH_PATH/workloads_spfs/${workload}_${thread}.f &> $output/result.txt
